...
class EPGRefreshConfiguration(Screen, HelpableScreen, ConfigListScreen):
	...
	def _getConfig(self):
		...
		if config.plugins.epgrefresh.enabled.value:
			...
			self.list.append(getConfigListEntry(_("Refresh EPG using"), config.plugins.epgrefresh.adapter, _("If you want to refresh the EPG in background, you can choose the method which best suits your needs here, e.g. hidden, fake recording or regular Picture in Picture."), False))
